  ...
  return (
    <section id="projects" className="py-24 scroll-mt-24 bg-white">
      ...
      >
        ...
        <h2 className="text-4xl md:text-5xl font-bold text-center mb-16 text-indigo-600">Projects</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {projects.map((p) => (
            <motion.div key={p.title} className="bg-white border border-gray-200 p-8 rounded-2xl shadow-lg text-center transition-transform duration-300 hover:-translate-y-1 hover:shadow-xl" variants={fadeInUp} whileHover={{ y: -4 }}>
              ...
              <h4 className="text-2xl font-semibold mb-4 text-indigo-600">{p.title}</h4>
              <div className="flex justify-center space-x-4">
                <a href={p.github} className="bg-white text-indigo-600 font-semibold py-2 px-4 rounded-full transition-colors duration-300 hover:bg-gray-50 border border-indigo-400" target="_blank" rel="noreferrer">GitHub</a>
                <a href={p.demo} className="bg-indigo-600 text-white font-semibold py-2 px-4 rounded-full transition-colors duration-300 hover:bg-indigo-700" target="_blank" rel="noreferrer">Live Demo</a>
              </div>
            </motion.div>
          ))}
        </div>
